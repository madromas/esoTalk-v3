...
function parseRequest($parts, $controllers)
{
	...
	if (!empty($parts[1])) {
		...
		if (str_contains($method, ".")) {
			[$method, $suffix] = explode(".", $method, 2);
		...
		foreach ($methods as $k => $v) {
			if (!str_starts_with($v = strtolower($v), "action_")) {
				unset($methods[$k]);
				continue;
			}
...
{
	$salt = "";
	for ($i = 0; $i < $numOfChars; $i++) $salt .= $possibleChars[random_int(0, strlen((string) $possibleChars) - 1)];
	return $salt;
}
...
function json_encode($a = false)
{
	...
	$isList = true;
	for ($i = 0, reset($a), $count = is_countable($a) ? count($a) : 0; $i < $count; $i++, next($a)) {
		if (key($a) !== $i) {
			$isList = false;
			break;
		}
	...
	$result = array();
	if ($isList) {
	  foreach ($a as $v) $result[] = json_encode($v, JSON_THROW_ON_ERROR);
	  return '['.implode(',', $result).']';
	} else {
	  foreach ($a as $k => $v) $result[] = '"'.($k).'":'.json_encode($v, JSON_THROW_ON_ERROR);
	  return '{'.implode(',', $result).'}';
	}
}
...
function json_decode($json)
{
	...
	$parts = preg_split("@(\"[^\"]*\")|([\[\]\{\},:])|\s@is", (string) $json, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
...
{
	if (str_starts_with($path, PATH_ROOT)) $path = substr($path, strlen((string) PATH_ROOT) + 1);
	return $path;
}
